...
export default function OptionButton(props){
  ...
  const setWasSelected = props.setWasSelected
    return(
      <button onClick={()=>{setOptions([...props.options, props.title]);setWasSelected(true)}} className={`w-28 h-28 sm:w-48 sm:h-48 ${props.color} rounded-full justify-center items-center flex ${props.winner}`}>
